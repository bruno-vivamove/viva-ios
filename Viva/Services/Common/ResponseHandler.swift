...
final class ResponseHandler<ErrorType: Decodable & Error> {
    ...
    init(decoder: JSONDecoder, shouldLogBodies: Bool = false, errorManager: ErrorManager? = nil) {
        ...
        case .failure(let error):
            if let tokenRefreshHandler = tokenRefreshHandler,
               let retryHandler = retryHandler {
                ...
                handleError(error, response: response, continuation: continuation)
            }
        }
    }
        ...
        case .failure(let error):
            if let tokenRefreshHandler = tokenRefreshHandler,
               let retryHandler = retryHandler {
                ...
                handleErrorWithoutResponse(error, response: response, continuation: continuation)
            }
        }
    }
        ...
        if let data = response.data,
           let errorResponse = try? decoder.decode(ErrorType.self, from: data) {
            ...
            if let urlError = error.underlyingError as? URLError {
                ...
                default:
                    networkError = NetworkClientError.requestError(message: error.localizedDescription)
                }
            ...
            errorManager?.displayError(networkError.userFriendlyMessage, type: .network)
           ...
           isUnauthorizedError(response.response) {
            ...
            Task {
                do {
                    ...
                    errorManager?.displayError("Session expired. Please log in again.", type: .authentication)
    ...
    func handleErrorWithoutResponse(
        ...
        if let tokenRefreshHandler = tokenRefreshHandler,
           ...
           isUnauthorizedError(response.response) {
            ...
            errorManager?.displayError("Your session has expired. Attempting to renew...", type: .authentication)
